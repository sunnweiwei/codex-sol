...
def load_official_config_dict() -> dict[str, Any]:
    ...
    # SYSTEM config has LOWER priority (merged first!)
    if sys_file.exists():
        try:
            config_dict = parse_simple_toml(sys_file.read_text(encoding="utf-8"))
        except Exception:
            pass
            
    # USER/DEVELOPER config under codex_home has HIGHER priority (merged second!)
    if dev_file.exists() and dev_file != sys_file:
        try:
            dev_dict = parse_simple_toml(dev_file.read_text(encoding="utf-8"))
            config_dict = deep_merge(config_dict, dev_dict)
        except Exception:
            pass
...
class CodexConfig:
    ...
    def resolved_codex_home(self) -> Path:
        ...
        py_home = os.environ.get("CODEX_PY_HOME")
        if py_home:
            return Path(py_home).absolute()
        c_home = os.environ.get("CODEX_HOME")
        if c_home:
            return Path(c_home).absolute()
        home = os.environ.get("HOME") or str(Path.home())
        return Path(home).absolute() / ".codex-python"
