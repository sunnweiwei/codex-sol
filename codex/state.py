...
def strip_memory_citations(text: str) -> tuple[str, list[str]]:
    ...
    th_match = re.search(r'<thread_ids>(.*?)</thread_ids>', citation_block, re.DOTALL)
    if th_match:
        for line in th_match.group(1).splitlines():
            trimmed = line.strip()
            if trimmed:
                citations.append(trimmed)
...
class CodexState:
    ...
    def rollout_path(self) -> Path:
        if getattr(self, "_rollout_path", None) is not None:
            return Path(self._rollout_path)
            ...
            if evt_type == "turn.started":
                ...
                mapped_payload = {
                    ...
                    "info": {
                        ...
                        "last_token_usage": payload.get("usage", {}),
                    }
                }
            elif evt_type == "tool.started":
                name = payload.get("name")
                call_id = payload.get("call_id")
                self._active_tools = getattr(self, "_active_tools", {})
                if name and call_id:
                    self._active_tools[call_id] = name
                    
                if name == "exec_command":
                    command = [payload.get("arguments", {}).get("cmd", "")]
                    cwd = str(Path(self.config.resolved_cwd()).resolve())
                    raw_actions = parse_command_actions(command[0])
                    parsed_cmd = []
                    for act in raw_actions:
                        new_act = dict(act)
                        if "command" in new_act:
                            new_act["cmd"] = new_act.pop("command")
                        parsed_cmd.append(new_act)
                    mapped_payload = {
                    ...
                    mapped_payload = {
                        ...
                        "call_id": call_id,
                        "path": str(Path(self.config.resolved_cwd()).resolve() / payload.get("arguments", {}).get("path", "")),
                    }
                elif name == "write_stdin":
                    session_id = str(payload.get("arguments", {}).get("session_id", ""))
                    stdin = payload.get("arguments", {}).get("chars", "")
                    target_call_id = "exec-1"
                    from codex.tools import ToolRuntime
                    if session_id in ToolRuntime._PROCESS_REGISTRY:
                        target_call_id = ToolRuntime._PROCESS_REGISTRY[session_id].get("call_id", "exec-1")
                    mapped_payload = {
                ...
                call_id = payload.get("call_id")
                self._active_tools = getattr(self, "_active_tools", {})
                name = self._active_tools.pop(call_id, None) if call_id else None
                ...
                if name == "exec_command":
                    is_exec_end = True
                    exit_code = payload.get("metadata", {}).get("exit_code", 0) if payload.get("metadata") else 0
                elif name == "write_stdin":
                    ...
                    if "exit_code" in meta:
                        ...
                        from codex.tools import ToolRuntime
                        if session_id in ToolRuntime._PROCESS_REGISTRY:
                            target_call_id = ToolRuntime._PROCESS_REGISTRY[session_id].get("call_id", call_id)
                            
                if is_exec_end:
                    duration_sec = 0.005
                    duration = {"secs": int(duration_sec), "nanos": int((duration_sec - int(duration_sec)) * 1000000000)}
                    
                    stdin_buf = ""
                    stdout_buf = ""
                    from codex.tools import ToolRuntime
                    for sid, info in list(ToolRuntime._PROCESS_REGISTRY.items()):
                        if info.get("call_id") == target_call_id:
                            stdin_buf = info.get("stdin_buffer", "")
                            stdout_buf = info.get("stdout_buffer", "")
                            break
                    ...
                    mapped_payload = {
                        ...
                        "aggregated_output": stdout_buf if stdout_buf else payload.get("output", ""),
                        "duration": duration,
                        "interaction_input": stdin_buf if stdin_buf else None,
                    }
                else:
                    mapped_payload = payload
            else:
    ...
    def ensure_session_meta_written(self) -> None:
        ...
        if getattr(self.config, "model_provider", None) is not None:
            meta_info["model_provider"] = self.config.model_provider
        ...
        # Replicate any loaded parent history to the brand new rollout file (skipping developer system prompts to avoid duplicates)
        if self.history:
        ...
        if item.get("type") == "message" and item.get("role") == "assistant":
            ...
            for part in content:
                if isinstance(part, dict) and "text" in part:
                    full_text += part["text"]
                    text_part = part
            if full_text and "<oai-mem-citation>" in full_text:
                visible_text, lines = strip_memory_citations(full_text)
                parsed = parse_memory_citation(lines)
                if parsed:
                    self.record_memory_citation(parsed)
                    if self.config.use_memories and getattr(self.config, "memory_state_store", None) is not None:
                        r_ids = parsed.get("rollout_ids", [])
                        if r_ids:
                            try:
                                self.config.memory_state_store.record_stage1_output_usage(r_ids)
                            except Exception:
                                pass
                if text_part is not None:
                    text_part["text"] = visible_text
