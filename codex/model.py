...
_STREAM_ACCUMULATED_ARGS: dict[str, str] = {}

def map_openai_sse_event_to_model_stream_event(openai_event: dict[str, Any]) -> ModelStreamEvent | list[ModelStreamEvent] | None:
    global _STREAM_ACCUMULATED_ARGS
    ev_type = openai_event.get("type")
    if ev_type == "response.created":
        # Reset the stream accumulated arguments on turn starting!
        _STREAM_ACCUMULATED_ARGS.clear()
    ...
    elif ev_type in ("response.output_item.delta", "response.function_call_arguments.delta"):
        item_id = openai_event.get("item_id") or openai_event.get("call_id") or "plan-1"
        delta_text = ""
        if "delta" in openai_event:
            ...
            delta_text = openai_event.get("text", "") or ""
            
        # Accumulate arguments
        _STREAM_ACCUMULATED_ARGS[item_id] = _STREAM_ACCUMULATED_ARGS.get(item_id, "") + delta_text
        
        return ModelStreamEvent("item.delta", {
            "item_id": item_id,
            "delta": delta_text,
            "aggregate": {
                "arguments": _STREAM_ACCUMULATED_ARGS[item_id]
            }
        })
    elif ev_type == "response.output_item.done":
        item = openai_event.get("item", {})
        item_id = item.get("id") or item.get("call_id")
        accumulated = _STREAM_ACCUMULATED_ARGS.get(item_id, "") or item.get("arguments", "") or ""
        ...
        return ModelStreamEvent("response.completed", {
            "usage": usage,
        })
    elif ev_type == "response.incomplete":
        resp = openai_event.get("response", {})
        usage = resp.get("usage", {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0})
        response_id = resp.get("id", "resp-1")
        ev1 = ModelStreamEvent("token_count", {
            "usage": usage,
            "info": {
        ...
        ev2 = ModelStreamEvent("model.failed", {
...
class ScriptedResponsesModel(ModelClient):
    ...
    def stream(self, request: PromptRequest) -> Iterable[ModelStreamEvent]:
        ...
        stream_list = resp.get("stream") or resp.get("events")
        if isinstance(stream_list, list):
            for ev in stream_list:
                # Try standard SSE event mapping
                if isinstance(ev, dict):
                    res = map_openai_sse_event_to_model_stream_event(ev)
                ...
                payload = ev.get("payload") if (isinstance(ev, dict) and "payload" in ev) else ev
                ev_type = ev.get("type", "unknown") if isinstance(ev, dict) else getattr(ev, "type", "unknown")
                yield ModelStreamEvent(type=ev_type, payload=payload)
        else:
            ...
            yield ModelStreamEvent("response.completed", {"usage": usage})
...
def iter_model_stream_events(events: Iterable[Any]) -> Iterable[ModelStreamEvent]:
    for ev in events:
        if isinstance(ev, ModelStreamEvent):
            ...
            ev_type = ev.get("type")
            if ev_type in ("response.created", "response.output_item.added", "response.output_item.delta", "response.output_item.done", "response.done", "response.incomplete"):
                res = map_openai_sse_event_to_model_stream_event(ev)
                if isinstance(res, list):
                    yield from res
                elif res is not None:
