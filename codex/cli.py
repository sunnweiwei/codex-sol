...
class _InteractiveSlashResult(Enum):
    HANDLED = "handled"
    IGNORED = "ignored"
    EXIT = "exit"
...
class _HumanEventRenderer:
    ...
    def finish(self, final_message: str, *, print_to_stdout: bool = True) -> None:
        ...
        if print_to_stdout:
            lines = _render_markdown_for_terminal(clean, self.style, terminal_width=90)
            self._write_line("")
            for l in lines:
                self._write_line(l)
            self._write_line("")
...
def _apply_prompt_escape_sequence(buffer: str, cursor: int, sequence: bytes) -> tuple[str, int] | None:
    if sequence == b'\x01':  # Ctrl-A
        return buffer, 0
    if sequence == b'\x05':  # Ctrl-E
...
def _background_terminal_rows(session: CodexSession) -> list[tuple[int, str, bool, str]]:
    rows = []
    for call_id, cmd_obj in session.tools._running_commands.items():
        pid = cmd_obj.proc.pid if (cmd_obj.proc and cmd_obj.proc.pid) else -1
        command_string = " ".join(cmd_obj.argv)
        is_running = (cmd_obj.proc.poll() is None) if cmd_obj.proc else False
        exit_code = cmd_obj.proc.returncode if cmd_obj.proc else -1
        status_string = "Running" if is_running else f"Exited ({exit_code})"
        rows.append((pid, command_string, is_running, status_string))
    return rows
...
def _rollout_picker_rows(config: CodexConfig) -> list[_RolloutPickerRow]:
    ...
    for path in sessions_dir.glob("**/*.jsonl"):
        ...
        try:
            ...
            thread_id = payload.get("id") or payload.get("session_id") or path.stem
            cwd = payload.get("cwd")
            git = payload.get("git", {})
...
def _rollout_picker_display_lines(
    ...
    *,
    ...
    lines = []
    filter_label = style.green(f"Filter: {query}") if query else style.dim("Filter: [all]")
    sort_label = style.cyan(f"Sort: {sort_key}")
    show_all_label = style.yellow("Show: All Workspaces") if show_all else style.dim("Show: Current CWD")
    lines.append(style.bold(f"=== {title} ==="))
    lines.append(f"  {filter_label}  |  {sort_label}  |  {show_all_label}")
    lines.append(style.dim("  Use [Up/Down] to select, [Tab] to focus toolbar, [Enter] to resume, [q] to exit"))
    lines.append("")
    ...
    for idx, row in enumerate(visible_slice):
        ...
        if is_sel:
            ...
            preview_str = f"     Preview: {row.preview}"
...
def _handle_interactive_slash_command(
    ...
    *,
    ...
    if cmd in ("/exit", "/quit"):
        ...
        renderer.finish(res.final_message)
        return _InteractiveSlashResult.HANDLED
...
def run_interactive_tui_picker(config: CodexConfig) -> Path | None:
    ...
    if not sys.stdin.isatty() or not sys.stdout.isatty():
        ...
        print("=== Codex Rollout Picker (Headless Fallback) ===")
    ...
    while True:
        ...
        lines = _rollout_picker_display_lines(
            rows=rows,
            title="Resume/Fork Session Rollout Picker",
            style=style,
            cwd=cwd,
            show_all=show_all,
            query=query,
...
def _main_chat(argv: list[str], *, prog: str = 'python -m codex') -> int:
    # Parses CLI
    import argparse
    parser = argparse.ArgumentParser(prog=prog, description="Codex CLI Python Port Agent")
    subparsers = parser.add_subparsers(dest="command")
    
    # 1. exec command
    exec_p = subparsers.add_parser("exec", help="Execute a single turn with the prompt")
    ...
    # 3. fork command
    fork_p = subparsers.add_parser("fork", help="Fork an existing session")
    fork_p.add_argument("--path", help="Path to rollout file (jsonl) to fork from")
    
    # General session arguments
    parser.add_argument("--model", help="OpenAI model to use (default: resolved from catalog)")
    parser.add_argument("--sandbox", choices=["workspace-write", "read-only", "danger-full-access"], default="workspace-write", help="Sandboxing policy")
    parser.add_argument("--approval-policy", choices=["always", "never", "unless-trusted"], default="never", help="Approval policy")
    parser.add_argument("--color", choices=["auto", "always", "none"], default="auto", help="ANSI Color mode")
    ...
    if args.command == "exec":
        ...
        if not rollout_path:
            # TUI picker resume!
            rollout_path = run_interactive_tui_picker(config)
            if not rollout_path:
                return 0
    ...
    while True:
        try:
            prompt = input(style.bold(style.green("Codex> "))).strip()
            if not prompt:
                continue
