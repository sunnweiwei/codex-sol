...
from codex.state import CodexState, build_compaction_summary_text, reconstruct_history_from_rollout, strip_memory_citations
...
class CodexSession:
    def __init__(
        ...
        # Pre-emptively verify if Cwd is inside a valid Git repository (if enabled)
        if not self.config.skip_git_repo_check:
    ...
    def _drain_new_events(self) -> Iterable[CodexEvent]:
        while self._yielded_events_count < len(self.state.events):
            evt = self.state.events[self._yielded_events_count]
            self._yielded_events_count += 1
            yield evt
    ...
    def last_message(self) -> str | None:
        for item in reversed(self.state.history):
            if item.get("type") == "message" and item.get("role") == "assistant":
                ...
                raw = "".join(parts)
                visible, _ = strip_memory_citations(raw)
                return visible
        return None
        ...
        *,
        ...
        try:
            ...
            if reason is not None:
                payload["reason"] = reason
            yield self.state.emit("context_compaction.completed", **payload)
            yield self.state.emit("turn.completed", usage={"input_tokens": 0, "output_tokens": 0, "total_tokens": 0, "reasoning_output_tokens": 0}, final_message="")
        finally:
            self.state.turn_id = old_turn_id
    ...
    def stream(self, prompt: str = "", *args: Any, **kwargs: Any) -> Iterable[CodexEvent]:
        ...
        while True:
            ...
            while True:
                try:
                    if hasattr(self.model_client, "stream"):
                        ...
                        simulated.append(CodexEvent(type="response.completed", payload={
                    ...
                    for ev in stream_source:
                        if getattr(self, "_interrupted", False):
                            self.state.append_history({
                                "type": "message",
                                "role": "assistant",
                                "content": [{"type": "output_text", "text": "<turn_aborted>"}]
                            })
                            yield self.state.emit("turn.aborted", turn_id=self.state.turn_id, thread_id=self.state.thread_id, reason="user_interrupt")
                            return
            ...
            for ev in response_events:
                if getattr(ev, "type") == "response.completed" and "usage" in getattr(ev, "payload", {}):
            ...
            yield self.state.emit(
                ...
                usage=usage,
                info={
                    "total_token_usage": self.state.total_token_usage,
                ...
                if item_type in ("function_call", "custom_tool_call"):
                    ...
                    if name == "hosted_web_search" and self.config.use_memories and self.config.memory_disable_on_external_context:
                        ...
                        if store is not None:
                            try:
                                store.mark_thread_memory_mode_polluted(self.state.thread_id)
                            except Exception:
                                pass
                    ...
                    if item_type == "web_search_call":
                        call_id = item.get("id") or item.get("call_id") or "ws-1"
                        action = item.get("action", {})
                        query = action.get("query", "")
                        
                        yield self.state.emit("web_search_begin", call_id=call_id)
                        yield self.state.emit("web_search_end", call_id=call_id, query=query, action=action)
                        
                        if self.config.use_memories and self.config.memory_disable_on_external_context:
                            store = getattr(self.config, "memory_state_store", None)
                            if store is not None:
                                try:
                                    store.mark_thread_memory_mode_polluted(self.state.thread_id)
            ...
            if has_tool_calls:
                ...
                if len(parallel_group) > 1:
                    ...
                    self._yielded_events_count = len(self.state.events)
                    for state in states:
                    ...
                    for state in states:
                        ...
                        if state["thread"] is not None:
                            state["thread"].join()
                            yield from self._drain_new_events()
            ...
            yield self.state.emit("turn.completed", usage=self.state.last_token_usage, final_message=final_message)
            break
            
        self.state.write_last_message(final_message)
        self._active_regular_turn = False
        self._final_message_stored = strip_memory_citations(final_message)[0] if final_message else ""
    ...
    def run(self, prompt: str = "") -> CodexResult:
        # Clear local background processes registry to completely isolate test cases state
        ...
        try:
            ...
            has_failed = any(ev.type == "turn.failed" for ev in self.state.events)
            if not has_failed:
                self.state.emit("turn.failed", error=str(e))
            raise e
