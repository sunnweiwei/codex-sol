...
class ToolRuntime:
    ...
    def apply_patch(self, arguments: Any, **kwargs: Any) -> ToolResult:
        ...
        cwd = self.config.resolved_cwd().resolve()
            ...
            for op in parsed_ops:
                ...
                if op_type == "add":
                    ...
                    if move_to:
                        ...
                        if not is_writable_path(abs_move, self.config):
                            escaped_paths.append(abs_move)
        ...
        # Generate diff & changes dict
        for abs_path, before_val in before_states.items():
            ...
            if before_val is None and after_val is not None:
                ...
                diff = "".join(difflib.unified_diff(
                    [], after_val.splitlines(keepends=True),
                    fromfile="", tofile=""
                ))
                ...
                # Delete or Rename source
                # find rename dest
                dest_rel = None
                for op in parsed_ops:
                    if op["type"] == "update" and op["move_to"] is not None:
                        if abs_path == (cwd / op["path"]).resolve():
                            dest_rel = op["move_to"]
                            break
                            
                if dest_rel:
                    after_move = after_states.get((cwd / dest_rel).resolve()) or ""
                    diff = "".join(difflib.unified_diff(
                        before_val.splitlines(keepends=True), after_move.splitlines(keepends=True),
                        fromfile="", tofile=""
                    ))
                    diff_lines = diff.splitlines(keepends=True)
                    unified = "".join(diff_lines[2:]) if len(diff_lines) > 2 else ""
                    ...
                    diff = "".join(difflib.unified_diff(
                        before_val.splitlines(keepends=True), [],
                        fromfile="", tofile=""
                    ))
                    diff_lines = diff.splitlines(keepends=True)
                    unified = "".join(diff_lines[2:]) if len(diff_lines) > 2 else ""
                ...
                if before_val != after_val:
                    diff = "".join(difflib.unified_diff(
                        before_val.splitlines(keepends=True), after_val.splitlines(keepends=True),
                        fromfile="", tofile=""
                    ))
                    diff_lines = diff.splitlines(keepends=True)
                    unified = "".join(diff_lines[2:]) if len(diff_lines) > 2 else ""
        ...
        if yield_time_ms is not None:
            ...
            try:
                ...
                if use_pty:
                    ...
                    # Close slave in parent since child owns it
                    os.close(slave_fd)
    ...
    def request_user_input(self, arguments: dict[str, Any], **kwargs: Any) -> ToolResult:
        ...
        if answers_payload is None:
            return ToolResult(ok=False, output="No user input provider or answer overrides are set in this session.")
            
        return ToolResult(ok=True, output=json.dumps(answers_payload))
